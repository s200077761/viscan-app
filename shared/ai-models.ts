...
export const AI_MODELS: Record<AIModelType, AIModel> = {
  ...
  'iris-scanner': {
    ...
    purpose: 'Analysis of iris patterns, colors, and potential health indicators',
  ...
  'health-predictor': {
    ...
    purpose: 'Predict health indicators and potential conditions from multiple data sources',
    technologies: ['Ensemble Models', 'Random Forest', 'XGBoost', 'Neural Networks'],
    initialAccuracy: 79,
    expectedAccuracy: 91,
    supportedImageTypes: ['xray', 'mri', 'ct', 'ultrasound', 'photo', 'document', 'other'],
  ...
  'gpt4-vision': {
    ...
    purpose: 'General-purpose medical image analysis with natural language understanding',
...
export function getRecommendedModel(imageType: string, bodyPart?: string): AIModelType {
  // Face analysis
  if (bodyPart?.toLowerCase().includes('face') || bodyPart?.toLowerCase().includes('head')) {
    return 'face-analyzer';
  }
  
  // Eye/Iris analysis
  if (bodyPart?.toLowerCase().includes('eye') || bodyPart?.toLowerCase().includes('iris')) {
    return 'iris-scanner';
  }
  
  // Hand/Palm analysis
  if (bodyPart?.toLowerCase().includes('hand') || bodyPart?.toLowerCase().includes('palm')) {
    return 'palm-reader';
  }
