...
export const IRIS_ZONES = {
  zone1: { name: "Stomach Zone", angle: [0, 51], organs: ["stomach", "digestive_system"] },
  zone2: { name: "Intestinal Zone", angle: [52, 103], organs: ["intestines", "colon"] },
  zone3: { name: "Kidney/Bladder Zone", angle: [104, 155], organs: ["kidneys", "bladder", "urinary_system"] },
  zone4: { name: "Heart/Lung Zone", angle: [156, 207], organs: ["heart", "lungs", "respiratory_system"] },
  zone5: { name: "Liver/Gallbladder Zone", angle: [208, 259], organs: ["liver", "gallbladder"] },
  zone6: { name: "Endocrine Zone", angle: [260, 311], organs: ["thyroid", "pituitary", "adrenal_glands"] },
  zone7: { name: "Brain/Nervous System Zone", angle: [312, 360], organs: ["brain", "nervous_system"] }
};
...
export const IRIS_SIGNS = {
  crypts: {
    ...
    health_implications: ["weakness in corresponding organ", "toxin accumulation", "inflammation"]
  },
  furrows: {
    description: "Radial lines extending from pupil",
    health_implications: ["nerve stress", "digestive issues", "chronic inflammation"]
  },
  spots: {
  ...
  rings: {
    ...
    health_implications: ["stress rings indicate tension", "cholesterol rings", "nerve rings"]
  },
  lacunae: {
    description: "Leaf-like openings",
    health_implications: ["organ weakness", "predisposition to disease", "hereditary factors"]
  },
  pigmentation: {
    description: "Color variations",
    health_implications: ["toxin accumulation", "mineral deposits", "metabolic issues"]
  }
};
...
export const IRIS_COLORS = {
  blue: {
    ...
    recommendations: ["alkaline diet", "lymphatic drainage", "anti-inflammatory foods"]
  },
  brown: {
    constitution: "Hematogenic",
    tendencies: ["blood disorders", "liver stress", "mineral imbalances"],
    recommendations: ["liver support", "blood purification", "mineral supplementation"]
  },
  mixed: {
    constitution: "Biliary",
    tendencies: ["digestive issues", "liver/gallbladder problems", "metabolic stress"],
...
interface IrisAnalysisResult {
  ...
  zones: Array<{
    ...
    zoneName: string;
    signs: IrisFeature[];
    healthStatus: "normal" | "mild_concern" | "moderate_concern" | "severe_concern";
    affectedOrgans: string[];
  }>;
 ...
 */
export async function analyzeIrisAdvanced(input: IrisAnalysisInput): Promise<IrisAnalysisResult> {
...
Provide detailed analysis for each of the 7 iris zones (0-51°, 52-103°, 104-155°, 156-207°, 208-259°, 260-311°, 312-360°).`
    ...
    response_format: {
      ...
      json_schema: {
        ...
        schema: {
            ...
            detectedSigns: {
              ...
              items: {
                ...
                properties: {
                  signType: {
                    type: "string",
                    enum: ["crypts", "furrows", "spots", "rings", "lacunae", "pigmentation"]
                  },
                  zoneNumber: {
                    type: "integer",
                    minimum: 1,
                ...
                required: ["signType", "zoneNumber", "clockPosition", "severity", "description"],
          ...
          required: ["irisColor", "pupilSize", "textureQuality", "detectedSigns", "overallQuality", "confidence"],
          additionalProperties: false
        }
      }
    }
  ...
  const zoneAnalysis = Object.entries(IRIS_ZONES).map(([zoneKey, zoneInfo]) => {
    ...
    const features: IrisFeature[] = signsInZone.map((sign: any) => ({
      ...
      healthImplications: IRIS_SIGNS[sign.signType as keyof typeof IRIS_SIGNS]?.health_implications || []
    }));
    ...
    const healthStatus: "normal" | "mild_concern" | "moderate_concern" | "severe_concern" = 
      hasSevere ? "severe_concern" :
      hasModerate ? "moderate_concern" :
      features.length > 0 ? "mild_concern" : "normal";
  ...
  const colorInfo = IRIS_COLORS[detection.irisColor as keyof typeof IRIS_COLORS] || IRIS_COLORS.mixed;
  ...
  zoneAnalysis.forEach(zone => {
    if (zone.healthStatus !== 'normal') {
      zone.affectedOrgans.forEach(organ => {
        recommendations.push(`Support ${organ.replace('_', ' ')} health through appropriate diet and lifestyle`);
      });
    }
  });
...
export function formatIrisAnalysis(result: IrisAnalysisResult) {
  ...
  const detailedMetrics = {
    ...
    zonesWithConcerns: result.zones.filter(z => z.healthStatus !== 'normal').length,
    totalSignsDetected: result.zones.reduce((sum, z) => sum + z.signs.length, 0),
  ...
  const hasModerate = result.zones.some(z => z.healthStatus === "moderate_concern");
