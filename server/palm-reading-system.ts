...
export const PALM_LINES = [
  {
    ...
    healthIndicators: {
      deep_and_clear: ['Strong vitality', 'Good immune system', 'High energy levels'],
      shallow: ['Low energy', 'Weak constitution', 'Need for rest'],
      broken: ['Health disruptions', 'Major illness periods', 'Recovery needed'],
  ...
  },
  {
    ...
    healthIndicators: {
      deep_and_straight: ['Clear thinking', 'Good mental health', 'Strong nervous system'],
  ...
  },
  {
    ...
    healthIndicators: {
      deep_and_clear: ['Strong heart', 'Good circulation', 'Emotional stability'],
      shallow: ['Weak heart', 'Poor circulation', 'Emotional coldness'],
      broken: ['Heart problems', 'Circulatory issues', 'Emotional trauma'],
      chained: ['Heart disease risk', 'Blood pressure issues', 'Emotional instability'],
  ...
  },
  {
    ...
    healthIndicators: {
      ...
      wavy: ['Chronic digestive issues', 'Liver problems', 'Metabolic disorders'],
    },
...
export const PALM_MOUNTS = [
  ...
  },
  {
    ...
    healthIndicators: {
      well_developed: ['Good communication', 'Healthy nervous system', 'Quick mind'],
  ...
  },
  {
    ...
    healthIndicators: {
      well_developed: ['Strong vitality', 'Good reproductive health', 'Passion'],
  ...
  },
  {
    ...
    healthIndicators: {
      well_developed: ['Good imagination', 'Healthy kidneys', 'Emotional balance'],
...
export const PALM_COLORS = {
  pink: { health: 'Excellent circulation', meaning: 'Good health', severity: 'normal' },
  red: { health: 'High blood pressure', meaning: 'Excess heat, anger', severity: 'moderate' },
  pale: { health: 'Anemia, poor circulation', meaning: 'Blood deficiency', severity: 'moderate' },
  yellow: { health: 'Liver problems', meaning: 'Bile issues, jaundice', severity: 'severe' },
  blue: { health: 'Poor circulation', meaning: 'Heart or lung issues', severity: 'severe' },
  white: { health: 'Severe anemia', meaning: 'Blood loss, weakness', severity: 'critical' },
};
...
export const PALM_TEXTURES = {
  soft: { health: 'Sedentary lifestyle', meaning: 'Need more activity', severity: 'mild' },
  firm: { health: 'Good muscle tone', meaning: 'Active lifestyle', severity: 'normal' },
  hard: { health: 'Overwork, tension', meaning: 'Need relaxation', severity: 'moderate' },
  dry: { health: 'Dehydration', meaning: 'Need more fluids', severity: 'mild' },
  moist: { health: 'Good hydration', meaning: 'Balanced fluids', severity: 'normal' },
  sweaty: { health: 'Nervous system stress', meaning: 'Anxiety, thyroid issues', severity: 'moderate' },
};
...
export function analyzePalmSigns(signs: PalmSign[]): PalmAnalysis {
  ...
  for (const sign of signs) {
    if (sign.type === 'line') {
      analyzePalmLine(sign, findings, systemsAffected, primaryConcerns, allRecommendations);
    } else if (sign.type === 'mount') {
      analyzePalmMount(sign, findings, systemsAffected, primaryConcerns, allRecommendations);
    } else if (sign.type === 'color') {
      analyzePalmColor(sign, findings, systemsAffected, primaryConcerns, allRecommendations);
    } else if (sign.type === 'texture') {
      analyzePalmTexture(sign, findings, systemsAffected, primaryConcerns, allRecommendations);
    }
  }

  // Determine overall health
  const severeConcerns = findings.filter(f => f.severity === 'severe').length;
  const moderateConcerns = findings.filter(f => f.severity === 'moderate').length;
...
function analyzePalmMount(
  ...
  const mount = PALM_MOUNTS.find(m => 
    sign.location.includes(m.nameEn.toLowerCase().replace(' ', '_').replace('/', '_'))
  );
...
function getRecommendationsForLine(line: string, characteristic: string): string[] {
  const recommendations: Record<string, Record<string, string[]>> = {
    'Life Line': {
      shallow: ['Increase rest', 'Boost immune system', 'Take vitamins'],
      broken: ['Consult doctor', 'Focus on recovery', 'Avoid stress'],
      chained: ['Improve digestion', 'Strengthen immunity', 'Regular check-ups'],
    },
    'Head Line': {
      wavy: ['Reduce stress', 'Practice meditation', 'Get adequate sleep'],
      broken: ['Rest your mind', 'Avoid overwork', 'Consult neurologist if needed'],
      chained: ['Mental health support', 'Stress management', 'Regular breaks'],
    },
    'Heart Line': {
      shallow: ['Cardiovascular exercise', 'Heart-healthy diet', 'Check blood pressure'],
      broken: ['Cardiology consultation', 'Monitor heart health', 'Reduce stress'],
    ...
    'Health Line': {
      ...
      wavy: ['Gastroenterology consultation', 'Metabolic panel', 'Dietary changes'],
    },
  };

  return recommendations[line]?.[characteristic] || ['Maintain healthy lifestyle'];
}

function getRecommendationsForMount(mount: string, characteristic: string): string[] {
  const recommendations: Record<string, Record<string, string[]>> = {
    'Mount of Jupiter': {
    ...
    'Mount of Apollo/Sun': {
      overdeveloped: ['Rest and recovery', 'Stress reduction', 'Heart monitoring'],
      flat: ['Cardiovascular exercise', 'Energy boosting foods', 'Iron supplement'],
    },
    'Mount of Mercury': {
      overdeveloped: ['Nervous system support', 'Magnesium supplement', 'Relaxation techniques'],
      flat: ['Brain foods', 'Mental stimulation', 'B-complex vitamins'],
    },
    'Mount of Venus': {
      overdeveloped: ['Energy conservation', 'Balanced lifestyle', 'Adequate rest'],
...
function getRecommendationsForColor(color: string): string[] {
  const recommendations: Record<string, string[]> = {
    red: ['Blood pressure check', 'Reduce stress', 'Cooling foods', 'Avoid spicy food'],
    pale: ['Iron supplement', 'Blood test', 'Iron-rich foods', 'Vitamin B12'],
    yellow: ['Liver function test', 'Avoid alcohol', 'Liver detox', 'Consult hepatologist'],
    blue: ['Circulation improvement', 'Cardiovascular check', 'Warm foods', 'Exercise'],
    white: ['Urgent blood test', 'Iron infusion may be needed', 'Medical consultation'],
...
function getRecommendationsForTexture(texture: string): string[] {
  const recommendations: Record<string, string[]> = {
    soft: ['Increase physical activity', 'Strength training', 'Active lifestyle'],
    hard: ['Relaxation techniques', 'Massage therapy', 'Reduce overwork'],
    dry: ['Increase water intake', 'Moisturize', 'Hydrating foods'],
    sweaty: ['Stress management', 'Thyroid check', 'Anxiety treatment if needed'],
