...
export class PDFExportService {
  ...
  private addBullet(text: string) {
    ...
    const lines = this.pdf.splitTextToSize(text, this.pageWidth - 2 * this.margin - 15);
  ...
  async exportAnalysisHistory(analyses: Analysis[], patientName?: string): Promise<void> {
    ...
    Object.entries(severityCounts).forEach(([severity, count]) => {
      this.addBullet(`${severity.charAt(0).toUpperCase() + severity.slice(1)}: ${count}`);
    });
    this.currentY += 5;
    ...
    analyses.forEach((analysis, index) => {
      ...
      this.pdf.setFont('helvetica', 'bold');
      this.pdf.text(`${index + 1}. ${analysis.modelName}`, this.margin + 3, this.currentY + 5);
      ...
      this.pdf.roundedRect(this.pageWidth - this.margin - 30, this.currentY + 1, 25, 6, 2, 2, 'F');
      this.pdf.setTextColor(255, 255, 255);
      this.pdf.setFontSize(8);
      this.pdf.text(
      ...
      // Analysis details
      this.pdf.setTextColor(0, 0, 0);
      this.addText(`Date: ${format(new Date(analysis.date), 'MMM dd, yyyy HH:mm')}`, 9);
  ...
  async exportRecommendations(
    ...
    if (patientName) {
      ...
      this.addText(`Name: ${patientName}`, 10, true);
      this.addText(`Report Date: ${format(new Date(), 'MMMM dd, yyyy')}`, 10);
      this.addText(`Overall Severity: ${severity.charAt(0).toUpperCase() + severity.slice(1)}`, 10);
      this.currentY += 5;
    }
    ...
    Object.entries(categoryCounts).forEach(([category, count]) => {
      this.addBullet(`${category.charAt(0).toUpperCase() + category.slice(1)}: ${count}`);
    });
    
    this.currentY += 3;
    this.addText('By Priority:', 10, true);
    Object.entries(priorityCounts).forEach(([priority, count]) => {
      this.addBullet(`${priority.charAt(0).toUpperCase() + priority.slice(1)}: ${count}`);
    });
    ...
    ['high', 'medium', 'low'].forEach(priority => {
      ...
      this.addSection(`${priority.charAt(0).toUpperCase() + priority.slice(1)} Priority Recommendations`);
      ...
      recs.forEach((rec, index) => {
        ...
        this.pdf.setFontSize(11);
        this.pdf.setFont('helvetica', 'bold');
        this.pdf.text(`${index + 1}. ${rec.title}`, this.margin + 3, this.currentY);
        this.currentY += 7;
        
        this.pdf.setFontSize(9);
        this.pdf.setFont('helvetica', 'italic');
        this.pdf.setTextColor(100, 100, 100);
        this.pdf.text(`Category: ${rec.category.charAt(0).toUpperCase() + rec.category.slice(1)}`, this.margin + 5, this.currentY);
        this.currentY += 7;
    ...
    this.pdf.save(`health-recommendations-${format(new Date(), 'yyyy-MM-dd')}.pdf`);
  }
  ...
  async exportCombinedReport(
    ...
    this.addText(`High Priority Items: ${recommendations.filter(r => r.priority === 'high').length}`, 10);
    this.currentY += 10;

    // Recent Analyses (last 5)
    this.addSection('Recent Analysis Results');
    analyses.slice(-5).reverse().forEach((analysis, index) => {
      this.checkPageBreak(25);
      this.addText(`${index + 1}. ${analysis.modelName}`, 10, true);
      this.addText(`   Date: ${format(new Date(analysis.date), 'MMM dd, yyyy')} | Severity: ${analysis.severity} | Confidence: ${analysis.confidence}%`, 9);
    ...
    this.pdf.save(`complete-health-report-${format(new Date(), 'yyyy-MM-dd')}.pdf`);
  }
}
